        ...
        if (confirmacao != null) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (retorno.RattingCliente.length > 0) {
                        var orders = localStorage.getItem('foca-gridorders');
                        if (orders != null) {
                            var obj = JSON.parse(orders);

                            $.each(obj.data, function (k, v) {
                                if (retorno.RattingCliente[0].IdRunKiLimpezaOperacao == v.IdPedido) {

                                    $('#panel-ratting').show(500);
                                    $('#txt-panel-ratting-nome').html(v.NomeProfissional);
                                    $('#txt-panel-ratting-datahora').html(v.DataPedido + ' - ' + v.HoraPedido + 'h (R$' + v.Valor + ')');
                                    $('#form-img-perfil-id').attr('name', v.IdPedido);

                                    FOCA.Model.Principal.getImagemList(v.IdProfissional, 'form-img-perfil-ratting');
                                }
                            });
                        }
                    }
