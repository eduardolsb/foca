FOCA = {
    ...
    Structure: {
        //url: 'http://201.65.0.71/Foca/Default.aspx',
        //url: 'http://192.168.0.196/Foca/Default.aspx',
        url: 'http://localhost:19088/Default.aspx',
        logout: function() {
            localStorage.clear();
    ...
    Methods: {
        ...
        isCelular: function(value, element) {
            value = value.replace("(", "");
            value = value.replace(")", "");
            value = value.replace("-", "");
        ...
        isTelefone: function(value, element) {
            value = value.replace("(", "");
            value = value.replace(")", "");
            value = value.replace("-", "");
            value = value.replace(" ", "").trim();
        ...
        isRatting: function (n) {
            for (var i = 1; i <= 5; i++) {
                if (i <= n) {
                    $('#ratting-' + i).attr('style', 'width: 15%; height: 40px; float: left; background: url(images/star_h.png) center center no-repeat; background-size: 50%; margin: 2%;');
                } else {
                    $('#ratting-' + i).attr('style', 'width: 15%; height: 40px; float: left; background: url(images/star.png) center center no-repeat; background-size: 50%; margin: 2%;');
                }
            }
            FOCA.Methods.ValueRatting = n;

            FOCA.Controller.Principal.functions.ratting($('#form-img-perfil-id').attr('name'), n); 
        },
        ValueRatting: 0,
        setFocaMSG: function (msg, timespeed) {
            var width = window.innerWidth;
            var height = window.innerHeight;
            var html = '' +
                '<div id="panel-foca-huhuuull" style="width:' + width + 'px;height:' + height + 'px;position:fixed;top:0px;left:-' + width + 'px;z-index:9999;">' +
                '   <div style="width:auto;max-width:90%;height:auto;background:#fff;border:solid 1px #222;border-radius:20px;margin-top: 50px;margin-left:10px;">' +
                '       <p style="padding:5px;margin:5px;"> ' + msg + ' </p>' +
                '   </div>' +
                '   <div style="width:50%;height:300px;background:url(../../../images/foca.gif) left center no-repeat;background-size: 200%;">' +
                '  </div>' +
                '</div>';
            $("body").append(html);
            ...
            if (msg.length > 20 && msg.length < 50) {
                tempo = 2000;
            }
            if (msg.length > 50) {
                tempo = 4000;
            ...
            timespeed = (timespeed == null) ? 200 : timespeed;

            $("#panel-foca-huhuuull").animate({
                'left': "+=" + width
            }, timespeed, function () {
                setTimeout(function() {
                    $("#panel-foca-huhuuull").animate({
